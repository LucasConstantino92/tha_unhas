...
class BookingRemoteDatasourceImpl implements BookingRemoteDatasource {
  ...
  Future<List<AppointmentModel>> getBookings(String userId) async {
    try {
      AppLogger.info(
        'Buscando agendamentos para o usuário ID: $userId no Supabase',
      ...
      final isAdmin =
          profileResponse != null && profileResponse['role'] == 'admin';

      final List<dynamic> response;
      if (isAdmin) {
        AppLogger.info(
          'Usuário é administrador. Buscando TODOS os agendamentos com detalhes do cliente/serviço.',
          'BookingRemoteDatasource',
        );
        ...
        AppLogger.info(
          'Usuário comum. Buscando apenas os agendamentos associados a ele.',
          'BookingRemoteDatasource',
        );
        response = await _supabaseClient
            .from('appointments')
      ...
      AppLogger.success(
        'Recuperados ${list.length} agendamentos',
        'BookingRemoteDatasource',
      );
      return list;
    } catch (e, stack) {
  ...
  Future<AppointmentModel> createBooking(AppointmentModel booking) async {
    try {
      AppLogger.info(
        'Criando agendamento para o usuário ID: ${booking.userId} no Supabase',
        'BookingRemoteDatasource',
      );
      ...
      AppLogger.success(
        'Agendamento criado com sucesso! ID: ${model.id}',
        'BookingRemoteDatasource',
      );
      return model;
    } catch (e, stack) {
      AppLogger.error(
        'Erro ao criar agendamento para o ID: ${booking.userId}',
        e,
        stack,
        'BookingRemoteDatasource',
      );
  ...
  Future<void> cancelBooking(String bookingId) async {
    try {
      AppLogger.info(
        'Tentando cancelar agendamento ID: $bookingId no Supabase',
        'BookingRemoteDatasource',
      );
      final helper = SBTables.appointments.helper(_supabaseClient);
      ...
      AppLogger.success(
        'Agendamento ID: $bookingId cancelado com sucesso',
        'BookingRemoteDatasource',
      );
    } catch (e, stack) {
      AppLogger.error(
        'Erro ao cancelar agendamento ID: $bookingId',
        e,
        stack,
        'BookingRemoteDatasource',
      );
